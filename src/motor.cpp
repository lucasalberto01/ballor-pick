...
void VespaMotors::backward(uint8_t speed) {
    ...
    this->_pwmA =
        map(speed, 0, 100, 0,
            this->_max_duty_cyle);  // transform to the current configuration
...
void VespaMotors::turn(int8_t speedA, int8_t speedB) {
    // update both speeds (the values and the directions are automatically
    // constrained)
...
void VespaMotors::_configurePWM(void) {
    ...
    this->_pwm_frequency = ledcSetup(this->_pwm_channel_A, this->_pwm_frequency,
                                     this->_pwm_resolution);
    this->_pwm_frequency = ledcSetup(this->_pwm_channel_B, this->_pwm_frequency,
                                     this->_pwm_resolution);
